...
async function openDevices() {
  ...
  try {
    ...
    list.querySelectorAll('.sync-btn').forEach((btn) => {
      btn.addEventListener('click', () => previewSync(btn.dataset.id, btn.dataset.name));
    });
  } catch {
    list.innerHTML = '<p style="color:#e91e63;text-align:center;padding:20px;">Server not reachable</p>';
  }
}
...
async function previewSync(deviceId, deviceName) {
  ...
  $('#sync-preview').innerHTML = '<p style="text-align:center;color:#adb5bd;padding:20px;">Loading...</p>';
  ...
  try {
    ...
    if (!data.items || data.items.length === 0) {
      $('#sync-preview').innerHTML = '<p style="text-align:center;color:#adb5bd;padding:20px;">No content on remote device</p>';
      return;
    }
    ...
    $('#sync-confirm').onclick = async () => {
      try {
        ...
        showStatus(`Synced ${result.itemsAdded} items from ${result.deviceName}`);
    ...
    $('#sync-preview').innerHTML = `<p style="color:#e91e63;text-align:center;padding:20px;">${escHtml(e.message)}</p>`;
  }
}
...
$('#devices-close').addEventListener('click', () => devicesModal.classList.add('hidden'));
$('#sync-close').addEventListener('click', () => syncModal.classList.add('hidden'));
$('#sync-cancel').addEventListener('click', () => syncModal.classList.add('hidden'));
$('#status-close').addEventListener('click', () => statusBar.classList.add('hidden'));
...
[editorModal, devicesModal, syncModal].forEach((modal) => {
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    [editorModal, devicesModal, syncModal].forEach((m) => m.classList.add('hidden'));
  }
});
